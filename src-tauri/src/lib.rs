...
// Simple-mode UI tweaks (nav links, footer copyright/version, gaps, about page,
// pdf-multi-tool header). Injected before page scripts; core/ stays untouched.
...
pub fn run() {
  tauri::Builder::default()
    .setup(|app| {
      ...
      // Expose the real app version to customize.js (footer version display).
      let version = app.package_info().version.to_string();
      let init_script = format!(
        "window.__SHOKADO_VERSION__={};\n{}",
        serde_json::to_string(&version).unwrap_or_else(|_| "\"0.1.0\"".into()),
        CUSTOMIZE_JS
      );
